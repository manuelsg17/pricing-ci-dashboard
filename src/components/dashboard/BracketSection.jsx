import { memo, useRef, useEffect, useMemo, useState, useCallback, lazy, Suspense } from 'react'
...
// `recharts` pesa ~97 KB gzip y MiniChart es su ÚNICO consumidor en el camino
// del Dashboard. Cargado con lazy(), esos 97 KB salen del arranque de la app y
// se descargan recién cuando hay un gráfico que dibujar. Importar recharts
// directamente acá otra vez revierte el ahorro sin que nada falle — el chunk
// simplemente vuelve a la carga inicial.
const MiniChart = lazy(() => import('./MiniChart'))

// Alto real de un chart (150px) + su leyenda, para que al aparecer el gráfico
// no salte el layout. Sin esta altura reservada, el Suspense colapsa a 0 y la
// página da un tirón cuando termina de cargar.
function ChartPlaceholder() {
  return <div className="chart-card" style={{ minHeight: 196 }} aria-hidden="true" />
}
    ...
    >
      ...
      {!collapsed && (
        <>
          ...
          {/* Charts. Suspense NO crea un nodo del DOM, así que los dos MiniChart
              siguen siendo hijos directos del grid de 2 columnas. */}
