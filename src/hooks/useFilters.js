...
import { getCountryConfig, resolveDbParams, getCompetitors } from '../lib/constants'
...
export function useFilters(country) {
  ...
  useEffect(() => {
    dbConfigsRef.current = dbConfigs
  }, [dbConfigs])

  const countryConfig = useMemo(() => getCountryConfig(country, dbConfigs), [country, dbConfigs])
  ...
  const [weekStart, setWeekStart] = useState(
    () => H.current['ws'] || toISODate(getMondayWeeksAgo(8))
  )
  const [dailyStart, setDailyStart] = useState(
    () => H.current['ds'] || toISODate(new Date(Date.now() - 6 * 86400000))
  )
  ...
  const [historicFrom, setHistoricFrom] = useState(
    () => H.current['hf'] || toISODate(getMondayWeeksAgo(24))
  )
  const [historicTo, setHistoricTo] = useState(
    () => H.current['ht'] || toISODate(getMondayWeeksAgo(0))
  )
  ...
  useEffect(() => {
    suppressCascades.current = false
  }, [])
