...
DECLARE
  ...
  v_err        text;
BEGIN
  -- 0. Pre-checks: tablas requeridas
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'bot_sync_log') THEN
    RAISE EXCEPTION 'Tabla bot_sync_log no existe — corre primero la migración 35_bot_sync_watermark.sql';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'bot_quotes_remote' AND relkind = 'f') THEN
    RAISE EXCEPTION 'Foreign table bot_quotes_remote no existe — corre primero la migración 36_bot_fdw_pipeline.sql';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = 'bot_rules') THEN
    RAISE EXCEPTION 'Tabla bot_rules no existe — corre primero la migración 37_bot_rules_table.sql';
  END IF;
