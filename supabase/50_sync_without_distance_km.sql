-- ════════════════════════════════════════════════════════════════════════
-- Migración 50 — Quitar distance_km del sync (el FDW no lo expone)
--
-- DESCUBRIMIENTO:
--   bot_quotes_remote NO tiene columna distance_km. La data del bot
--   viene solo con distance_bracket pre-clasificado. Las migraciones
--   46/47/49 incluían distance_km en el SELECT del FDW y eso producía
--   ERROR 42703 "column does not exist".
--
-- ESTRATEGIA:
--   Confiar en distance_bracket que el bot ya emite. La función
--   normalize_distance_bracket() (mig 47) colapsa las variantes
--   zone-aware (long_a, *_zona_sur, airport_*, etc.) a los 6 canónicos.
--   distance_km se queda NULL en pricing_observations — solo las
--   filas insertadas via Excel manual o data entry tienen ese valor.
-- ════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  VALUES (p_country, 'running',
          jsonb_build_object('source', 'sync_bot_quotes_v50', 'limit', p_limit))
  ...
  -- INSERT sin distance_km — queda NULL en pricing_observations (OK,
  -- el dashboard solo necesita distance_bracket que sí viene del bot
  -- y se normaliza vía normalize_distance_bracket)
