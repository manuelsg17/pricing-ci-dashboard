...
// Cuentas PROPIAS de la suite, distintas de admin@local.test (revisión
...
// aviso. Con cuentas dedicadas, los DELETE de limpieza de los tests nunca
// pueden tocar datos de otra persona.
//
// Dos cuentas: la segunda existe SOLO para el escenario de dos hubs
// concurrentes sobre el mismo frente (e2e/ci-tuktuk-y-dos-hubs.spec.js).
export const E2E_USERS = [
  { email: 'e2e-ci@local.test', firstName: 'E2E', stateFile: './e2e/.auth/admin.json' },
  { email: 'e2e-ci-2@local.test', firstName: 'E2E-2', stateFile: './e2e/.auth/hub2.json' },
]
...
async function ensureUser(admin, { email, firstName }) {
  ...
  if (profileErr) {
    throw new Error(
      `[e2e/global-setup] No se pudo crear el perfil de ${email}: ${profileErr.message}`
    )
  }
}

async function sessionFor(admin, email) {
...
function writeState(stateFile, session) {
...
export default async function globalSetup() {
  ...
  for (const u of E2E_USERS) {
    ...
    writeState(u.stateFile, await sessionFor(admin, u.email))
  }
}
